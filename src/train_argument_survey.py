...
if __name__ == '__main__':   
    ...
    group_list = country_and_group_df['Country'].tolist()
    
    for group in group_list:
        names = country_and_group_df['Country'].tolist()
        name_idx = names.index(group)
        ...
        epoch_num = _find_save_path(f"./ckpt/argument/{model_name}/TH_{threshold}/{group}")
        peft_model_id = f"./ckpt/argument/{model_name}/TH_{threshold}/{group}/{epoch_num}"
        ...
        for epoch in range(num_epochs):
            ...
            if eval_epoch_loss < best_loss:
                peft_model_id = f"./ckpt/argument_survey/{model_name}/{strategy}_TH_{threshold}/{country}/epoch_{epoch+1}"
